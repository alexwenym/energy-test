...
double compute_statistic(const std::vector<Event> &data_1, const std::vector<Event> &data_2, const bool debug = false) {
  ...
  sum_moments dist_1122; 
  dist_1122.sum = dist_11.sum + dist_22.sum; 
  dist_1122.sum2 = dist_11.sum2 + dist_22.sum2;
  dist_1122.sum3 = dist_11.sum3 + dist_22.sum3;
  dist_1122.sum4 = dist_11.sum4 + dist_22.sum4;

  dist_1122.sum  /= (data_2.size() * (data_2.size() - 1)) + (data_1.size() * (data_1.size() - 1)); 
  dist_1122.sum  *= 2; 
  dist_1122.sum2 /= (data_2.size() * (data_2.size() - 1)) + (data_1.size() * (data_1.size() - 1));
  dist_1122.sum2 *= 2;
  dist_1122.sum3 /= (data_2.size() * (data_2.size() - 1)) + (data_1.size() * (data_1.size() - 1)); 
  dist_1122.sum3 *= 2; 
  dist_1122.sum4 /= (data_2.size() * (data_2.size() - 1)) + (data_1.size() * (data_1.size() - 1));
  dist_1122.sum4 *= 2;

  double var_1122 = dist_1122.sum2 - (dist_1122.sum*dist_1122.sum);
  double stdev_1122 = sqrt(var_1122); 
  double skewness_1122 = dist_1122.sum3 - (3*dist_1122.sum)*dist_1122.sum2 + 2*dist_1122.sum*dist_1122.sum*dist_1122.sum; 
  skewness_1122 /= stdev_1122*stdev_1122*stdev_1122; 
  double kurtosis_1122 = dist_1122.sum4 - (4*dist_1122.sum)*dist_1122.sum3 + (6*dist_1122.sum*dist_1122.sum)*dist_1122.sum2 - 3*pow(dist_1122.sum,4);
  kurtosis_1122 /= var_1122*var_1122;
  kurtosis_1122 -= 3; 


  sum_moments dist_12 = compute_distance(data_1, data_2, false);
    
  dist_12.sum  /= data_1.size() * data_2.size(); 
  dist_12.sum2 /= data_1.size() * data_2.size(); 
  dist_12.sum3 /= data_1.size() * data_2.size(); 
  dist_12.sum4 /= data_1.size() * data_2.size(); 
    
  double var_12 = dist_12.sum2 - (dist_12.sum*dist_12.sum);
  double stdev_12 = sqrt(var_12); 
  double skewness_12 = dist_12.sum3 - (3*dist_12.sum)*dist_12.sum2 + 2*dist_12.sum*dist_12.sum*dist_12.sum; 
  skewness_12 /= stdev_12*stdev_12*stdev_12; 
  double kurtosis_12 = dist_12.sum4 - (4*dist_12.sum)*dist_12.sum3 + (6*dist_12.sum*dist_12.sum)*dist_12.sum2 - 3*pow(dist_12.sum,4);
  kurtosis_12 /= var_12*var_12;
  kurtosis_12 -= 3; 


  double statistic; 
  double sum_diff = (dist_1122.sum-dist_12.sum); 
  double rms_diff = (sqrt(dist_1122.sum2) - sqrt(dist_12.sum2)); 
  double stdev_diff = (stdev_1122-stdev_12);
  double skewness_diff = skewness_1122 - skewness_12; 
  double kurtosis_diff = kurtosis_1122 - kurtosis_12; 

  statistic = sum_diff*sum_diff/(var_1122 + var_12) +
      //2*rms_diff*rms_diff/(var_1122 + var_12); 
      2*stdev_diff*stdev_diff/(var_1122 + var_12); 
      //skewness_diff*skewness_diff/12 + 
      //kurtosis_diff*kurtosis_diff/48; 

  if (debug) {
      std::cout << "        mean contribution: " << sum_diff*sum_diff/(var_1122 + var_12) << std::endl; 
      std::cout << "        stdev contribution: " << 2*stdev_diff*stdev_diff/(var_1122 + var_12) << std::endl; 
      //std::cout << "        RMS contribution: " << 2*rms_diff*rms_diff/(var_1122 + var_12) << std::endl;
