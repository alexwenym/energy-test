...
// main function
void plotting(string infile1name, string infile2name, Int_t nbins, Double_t delta) {
// two input files 
...
TH3D * scatter1 = new TH3D("scatter1", "", 1000, 0, 1, 1000, 0, 1, 1000, 0, 1); 
TH3D * scatter2 = new TH3D("scatter2", "", 1000, 0, 1, 1000, 0, 1, 1000, 0, 1); 
...
for(unsigned j=0; j<s1vec.size(); j++) {
    ...
    scatter1->Fill(s1vec[j].m1, s1vec[j].m2, s1vec[j].m3); 
    scatter2->Fill(s2vec[j].m1, s2vec[j].m2, s2vec[j].m3); 
}
...
TCanvas * canvas1 = new TCanvas("canvas1", "", 1200, 600); 
canvas1->Divide(2,1); 
canvas1->cd(1); 
shist1->Draw(); 
canvas1->cd(2); 
...
TCanvas * canvas2 = new TCanvas("canvas2", "", 1200, 600); 
canvas2->Divide(2,1); 
canvas2->cd(1); 
scatter1->Draw(); 
canvas2->cd(2); 
