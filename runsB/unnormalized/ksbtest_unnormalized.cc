...
// main function
Int_t ksbtest_unnormalized(string infile1name, string infile2name, Int_t nbins, Int_t npermutations, Double_t delta) {
// two input files 
...
canvas0->cd(1); 
gPad->SetLogy(); 
disthist1->GetCumulative("_cu")->Draw("hist"); 
canvas0->cd(2); 
gPad->SetLogy(); 
disthist2->GetCumulative("_cu")->Draw("hist");

TCanvas * diffcanvas = new TCanvas("diffcanvas","",700,500); 
diffcanvas->cd();
disthist1->GetCumulative()->Add(disthist2->GetCumulative(),-1); 
//TH1F diffcuhist;
