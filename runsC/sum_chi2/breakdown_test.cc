...
shist->Scale(1.0/0.3998000); 
chist->Scale(1.0/0.4000000); 
...
TH1F* tcontributions = new TH1F("tcontributions","",shist_xbins,0,1);
...
for (Int_t p = 0; p<npermutations; p++) {
    ...
    auto loop_pure_histo(vector<event_t> s1vec, vector<event_t> s2vec) {
        
        for(unsigned j=0; j<s1vec.size(); j++) {

            for(unsigned k=j+1; k<s2vec.size(); k++) {
    ...
    shist->Scale(1.0/0.39980000); 
    chist->Scale(1.0/0.40000000); 
    
    TH1F* sumhist = (TH1F*)shist->Clone(); 

    sumhist->Add(chist, -1);
    *sumhist = (*sumhist)*(*sumhist); 
    Double_t tvalue = sumhist->Integral();  
